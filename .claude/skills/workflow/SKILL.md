---
name: workflow
description: Universal per-task state machine with gate-failure feedback routing, task graph, outer-loop context rules, small-model discipline, and the self-improvement protocol. Use for ANY nontrivial task - features, bugs, docs, infra - and always when task-loop or wf-run selects a task.
---

# workflow

You (the main session) are the orchestrator: delegate each state to its subagent (planner, qa-tester, verifier, code-reviewer, doc-manager, retro) via the Task tool, execute IMPLEMENT and COMMIT yourself, and route feedback per the table below. task-loop or the wf-run.sh driver selects WHICH node; this skill executes it.
