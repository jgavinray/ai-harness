...
# Fail-open detector: warn into context if registered hook scripts are missing/non-executable.
SELF_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
for h in pretool-guard.sh post-edit-check.sh stop-gate.sh; do
  [ -x "$SELF_DIR/$h" ] || echo "WARNING: hook $h missing or not executable in $SELF_DIR — that guard is silently ABSENT (hooks fail open)"
done
